                ...
                if (selectedFile) {
                  const newFiles = myFiles.map(file => {
                    if (file.id === selectedFile.id) {
                      return {
                        ...file,
                        name: prompt("Enter new name",file.name) ?? file.name
                      }
                    }
                    return file
                  })
            ...
            <button style={styles.controlButton}
              onClick={() => {
                if (selectedFile) {
                  ...
                  link.download = selectedFile.name; // Set the desired file name
                  link.target = "_blank";
                  link.click();
                }
              }}
            ...
            <div style={{ width: "100%", padding: 10 }}>
              {myFiles.map((file) => {
                ...
                if (file.path.slice(0, filePath.length) === filePath) {
                  return (
                    <div style={selectedFile?.path === file.path ? styles.fileSelected : styles.file} className="files" key={file.id} onClick={() => {
